  ...
  <div class="container">
    <div class="row">
      <div class="col-md-12">
        <div class="row">
          <div
            class="col-md-3 product-box m-4 bg-white p-2"
          ...
          >
            <router-link :to="{ path: '/beers/' + beer.id }">
              <img
                :src="beer.image"
                @error="beer.image = '/beers/default.png'"
                :alt="beer.name"
                class="img-fluid"
              />
              <h5>
                <span v-html="beer.name"></span>
            ...
            <div class="row p-2">
              <input
                ...
                :id="`units_${beer.id}`"
                value="1"
                v-model="units[beer.id]"
                :set="units[beer.id] == null ? (units[beer.id] = 1) : null"
                type="number"
                min="1"
                max="100"
                required
              />
              <button
                class="col-md-6 btn btn-sm btn-primary float-right"
                @click="addToCart(beer.id)"
              >
                Add to Cart
              </button>
            </div>
